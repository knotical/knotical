...
module EList = Extlib.ExtList.List
...
module Print = struct
  ...
  let pr_to_string (printer: Format.formatter -> 'a -> unit) (input: 'a): string =
    let fmt = Format.str_formatter in
    let () = printer fmt input in
    Format.flush_str_formatter ()

    (* let tmp_file = "tmp" in
     * let tmp_stdout = open_out tmp_file in
     * let () =
     *   Format.set_formatter_out_channel tmp_stdout;
     *   printer input;
     *   Format.print_newline ();
     *   Format.set_formatter_out_channel stdout
     * in
     * let s = BatFile.with_file_in tmp_file BatIO.read_all in
     * s *)

  let pr_core_time = Core.Time.Span.to_string
end;;
...
module Func = struct
  ...
  let core_time (f: unit -> 'a): ('a * Core.Time.Span.t) =
    let start = Core.Time.now () in
    let res = f () in
    let stop = Core.Time.now () in
    (res, Core.Time.diff stop start)
...
module LinReg = struct
  ...
  let rec gradient_descent_series (weights : int list) =
    ...
    Debug.trace_1 "gradient_descent_series"
