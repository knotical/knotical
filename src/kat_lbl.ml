 ...
 * 
 *   let rec kat_to_test (ke: Kat.test): test =
 *     match ke with
 *     | Kat.Dsj (l, r) -> Dsj (kat_to_test l, kat_to_test r)
 *     | Kat.Cnj (l, r) -> Cnj (kat_to_test l, kat_to_test r)
 *     | Kat.Neg e -> Neg (kat_to_test e)
 *     | Kat.Top -> Top
 *     | Kat.Bot -> Bot
 *     | Kat.Prd k -> Prd (K.from_kat k)
 * 
 *   let rec kat_to_expr (ke: Kat.expr): expr =
 *     match ke with
 *     | Kat.Pls (l, r) -> Pls (kat_to_expr l, kat_to_expr r)
 *     | Kat.Dot (l, r) -> Dot (kat_to_expr l, kat_to_expr r)
 *     | Kat.Str e -> Str (kat_to_expr e)
 *     | Kat.Tst e -> Tst (kat_to_test e)
...
struct
  ...
  let from_string s =
    ...
    | k::l::[] -> (L.from_string l, String.get k 0)
    | _ -> error ("LKey.from_string: Unexpected string " ^ s)
...
struct
  ...
  let cmp (_, k1) (_, k2) = Char.compare k1 k2
