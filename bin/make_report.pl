...
foreach my $b (sort @bench) {
    ...
    print IDX "<tr style=\"border-top: 1pt solid black;\"><td><pre>$b</pre><td><a href=\"$URLBASE/results-$STAMP/kn-$b.html\">$b</a></td><td>$loc</td><td>$stats_kv{procs}</td><td>$cmpltIDX</td><td>$Tim</td><td>$Rs</td><td>$digest{mintuples}</td><td>$digest{maxtuples}</td><td>$digest{minaxioms}</td><td>$digest{maxaxioms}</td></tr>\n";
