  ...
  let compute
	...
	if !Opt.iteration_guided then
	  Fixpoint.analysis_guided
		...
		~widening_start:(!Opt.widening_start)
		~widening_descend:(!Opt.widening_descend)
		~depth:(!Opt.iteration_depth)
		graph sstart)
	else
...
let print_abstract1 fmt abs =
  if !Opt.print_box then
    ...
    (* let man = Apron.Abstract1.manager abs in
     * if Apron.Abstract1.is_bottom man abs then
     *   fprintf fmt "%s" "false"
     * else if Apron.Abstract1.is_top man abs then
     *   fprintf fmt "%s" "true"
     * else
     *   let lincons_array = Apron.Abstract1.to_lincons_array man abs in
     *   (\* let lincons_list = Array.to_list lincons_array.Apron.Lincons1.lincons0_array in
     *    * PSpl_syn.print_list ~first:"@[" ~sep:",@ " ~last:"@]"
     *    *   (fun fmt -> Apron.Lincons0.print (fun _ -> "") fmt) fmt lincons_list *\)
     *   Apron.Lincons1.array_print fmt lincons_array *)
    ...
    (PSpl_syn.print_program
       begin fun fmt (point:Spl_syn.point) ->
	       ...
	       fprintf fmt "@[<hov>%s%a@ %a%s@]"
	         (!Opt.displaytags).Opt.precolorR
	         PSpl_syn.print_point point
	         print_abstract1 abs
	         (!Opt.displaytags).Opt.postcolor
       end)
    prog
